/*
  SPI TFT interface for Arduino TSDesktop
...
// Possible values for TSD_SPITFT.connection:
...
/*!
    @brief   TSD_SPITFT constructor for software (bitbang) SPI.
...
/*!
    @brief   TSD_SPITFT constructor for hardware SPI using the board's
...
TSD_SPITFT::TSD_SPITFT(int8_t cs, int8_t dc, int8_t rst)
    : connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc) {
  hwspi._spi = &SPI;
}
#else  // !ESP8266
TSD_SPITFT::TSD_SPITFT(int8_t cs, int8_t dc, int8_t rst)
    : TSD_SPITFT(&SPI, cs, dc, rst) {
...
/*!
    @brief   TSD_SPITFT constructor for hardware SPI using a specific
             SPI peripheral.
...
*/
TSD_SPITFT::TSD_SPITFT(SPIClass *spiClass, int8_t cs, int8_t dc, int8_t rst)
    : connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc) {
  hwspi._spi = spiClass;
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
...
/*!
    @brief   TSD_SPITFT constructor for parallel display connection.
    @param   w         Display width in pixels at default rotation (0).
    @param   h         Display height in pixels at default rotation (0).
...
*/
void TSD_SPITFT::initSPI(uint32_t freq, uint8_t spiMode) {
...
*/
void TSD_SPITFT::setSPISpeed(uint32_t freq) {
#if defined(SPI_HAS_TRANSACTION)
...
*/
void TSD_SPITFT::startWrite(void) {
...
*/
void TSD_SPITFT::writePixel(clip_t* clip, int16_t x, int16_t y, const rgb_t color) {
...
*/
void TSD_SPITFT::swapBytes(uint16_t *src, uint32_t len, uint16_t *dest) {
  if (!dest)
...
void TSD_SPITFT::writePixels(const rgb_t *colors, uint32_t len, bool block,
  ...
  if (!bigEndian) {
    ...
    hw_write_masked(&spi_get_hw(pi_spi)->cr0, 15 << SPI_SSPCR0_DSS_LSB,
                    SPI_SSPCR0_DSS_BITS);
    spi_write16_blocking(pi_spi, (uint16_t*)colors, len);
...
*/
bool TSD_SPITFT::dmaBusy(void) const {
#if defined(USE_SPI_DMA) && (defined(__SAMD51__) || defined(ARDUINO_SAMD_ZERO))
  return dma_busy;
#else
  return false;
...
*/
void TSD_SPITFT::writeColor(rgb_t color, uint32_t len) {
...
void TSD_SPITFT::writeFillRect(clip_t* clip, int16_t x, int16_t y, int16_t w,
  ...
  if (w && h) {   // Nonzero width and height?
    ...
    if (x < clip->x2) { // Not off right
      if (h < 0) {    // If negative height...
        y += h + 1;   //   Move Y to top edge
        h = -h;       //   Use positive height
      }
      if (y < clip->y2) { // Not off bottom
        int16_t x2 = x + w - 1;
        if (x2 >= clip->x1) { // Not off left
          int16_t y2 = y + h - 1;
          if (y2 >= clip->y1) { // Not off top
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void TSD_SPITFT::pushColor(const rgb_t color) {
  startWrite();
  SPI_WRITE16(color);
  endWrite();
}
...
void TSD_SPITFT::drawRGBBitmap(clip_t *clip, int16_t x, int16_t y,
  ...
  if (x < clip->x1) {              // Clip left
    bx1 = clip->x1 - x;
    w -= bx1;
    x = clip->x1;
  }
  if (y < clip->y1) { // Clip top
    by1 = clip->y1-y;
    h -= by1;
    y = clip->y1;
  }
  if (x2 >= clip->x2)
    w = clip->x2 - x; // Clip right
  if (y2 >= clip->y2)
    h = clip->y2 - y; // Clip bottom
...
*/
void TSD_SPITFT::invertDisplay(bool i) {
  startWrite();
...
*/
uint16_t TSD_SPITFT::color565(uint8_t red, uint8_t green, uint8_t blue) {
  return ((red & 0xF8) << 8) | ((green & 0xFC) << 3) | (blue >> 3);
}

/*!
@brief   TSD_SPITFT Send Command handles complete sending of commands and
data
@param   commandByte       The Command Byte
@param   dataBytes         A pointer to the Data bytes to send
@param   numDataBytes      The number of bytes we should send
*/
void TSD_SPITFT::sendCommand(uint8_t commandByte, uint8_t *dataBytes,
...
/*!
 ...
 @param   numDataBytes      The number of bytes we should send
 */
void TSD_SPITFT::sendCommand(uint8_t commandByte, const uint8_t *dataBytes,
                                  uint8_t numDataBytes) {
  SPI_BEGIN_TRANSACTION();
  if (_cs >= 0)
    SPI_CS_LOW();
...
/*!
 @brief  TSD_SPITFT sendCommand16 handles complete sending of
 ...
 @param  numDataBytes  The number of bytes we should send
 */
void TSD_SPITFT::sendCommand16(uint16_t commandWord,
 ...
 */
/**************************************************************************/
uint8_t TSD_SPITFT::readcommand8(uint8_t commandByte, uint8_t index) {
  uint8_t result;
  startWrite();
  SPI_DC_LOW(); // Command mode
 ...
 */
uint16_t TSD_SPITFT::readcommand16(uint16_t addr) {
...
*/
inline void TSD_SPITFT::SPI_BEGIN_TRANSACTION(void) {
  if (connection == TFT_HARD_SPI) {
#if defined(SPI_HAS_TRANSACTION)
...
*/
inline void TSD_SPITFT::SPI_END_TRANSACTION(void) {
#if defined(SPI_HAS_TRANSACTION)
  if (connection == TFT_HARD_SPI) {
...
*/
void TSD_SPITFT::spiWrite(uint8_t b) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(b);
#elif defined(ESP8266) || defined(ESP32)
...
*/
void TSD_SPITFT::writeCommand(uint8_t cmd) {
  SPI_DC_LOW();
  spiWrite(cmd);
  SPI_DC_HIGH();
}
...
*/
uint8_t TSD_SPITFT::spiRead(void) {
  uint8_t b = 0;
  uint16_t w = 0;
  if (connection == TFT_HARD_SPI) {
...
*/
void TSD_SPITFT::write16(uint16_t w) {
  if (connection == TFT_PARALLEL) {
#if defined(USE_FAST_PINIO)
    if (tft8.wide)
...
*/
void TSD_SPITFT::writeCommand16(uint16_t cmd) {
  SPI_DC_LOW();
  write16(cmd);
  SPI_DC_HIGH();
}
...
/*!
    ...
    @return  Unsigned 16-bit value read (always zero if USE_FAST_PINIO is
             not supported by the MCU architecture).
*/
uint16_t TSD_SPITFT::read16(void) {
  uint16_t w = 0;
  if (connection == TFT_PARALLEL) {
    if (tft8._rd >= 0) {
#if defined(USE_FAST_PINIO)
...
*/
inline void TSD_SPITFT::SPI_MOSI_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.mosiPortSet = 1;
...
*/
inline void TSD_SPITFT::SPI_MOSI_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.mosiPortClr = 1;
...
*/
inline void TSD_SPITFT::SPI_SCK_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.sckPortSet = 1;
...
*/
inline void TSD_SPITFT::SPI_SCK_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *swspi.sckPortClr = 1;
...
*/
inline bool TSD_SPITFT::SPI_MISO_READ(void) {
#if defined(USE_FAST_PINIO)
#if defined(KINETISK)
...
/*!
    ...
    @param  w  16-bit value to write.
*/
void TSD_SPITFT::SPI_WRITE16(uint16_t w) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(w >> 8);
    AVR_WRITESPI(w);
...
*/
void TSD_SPITFT::SPI_WRITE32(uint32_t l) {
  if (connection == TFT_HARD_SPI) {
#if defined(__AVR__)
    AVR_WRITESPI(l >> 24);
    AVR_WRITESPI(l >> 16);
...
*/
inline void TSD_SPITFT::TFT_WR_STROBE(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
#if defined(KINETISK)
  *tft8.wrPortClr = 1;
...
/*!
    @brief  Set the RD line HIGH. Used for parallel-connected interfaces
            when reading data.
*/
inline void TSD_SPITFT::TFT_RD_HIGH(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
...
/*!
    @brief  Set the RD line LOW. Used for parallel-connected interfaces
            when reading data.
*/
inline void TSD_SPITFT::TFT_RD_LOW(void) {
#if defined(USE_FAST_PINIO)
#if defined(HAS_PORT_SET_CLR)
