...
void loop() {
  ...
  tft.fillCircle(xpos, ypos, radius + 1, NAVY);
...
void ringMeter(int x, int y, int r, int val, const char *units)
{
  ...
  if (initMeter) {
    ...
    tft.drawSmoothCircle(x, y, r, SILVER, DARKER_GREY);
    uint16_t tmp = r - 3;
    tft.drawArc(x, y, tmp, tmp - tmp / 5, last_angle, 330, BLACK, DARKER_GREY);
  }
  ...
  if (last_angle != val_angle) {
    ...
    if (val_angle > last_angle) {
      tft.drawArc(x, y, r, r - thickness, last_angle, val_angle, SKYBLUE, BLACK); // SKYBLUE random(0x1000000)
    }
    else {
      tft.drawArc(x, y, r, r - thickness, val_angle, last_angle, BLACK, DARKER_GREY);
    }
