...
static inline void pio_sm_clr_fifo_debug_flags(PIO pio, uint sm)
{
...
typedef struct {
    ...
    #define PINHI_ALL_PINCTRL_LSBS ( \
        (1u << PIO_SM0_PINCTRL_IN_BASE_LSB) | \
        (1u << PIO_SM0_PINCTRL_OUT_BASE_LSB) | \
        (1u << PIO_SM0_PINCTRL_SET_BASE_LSB) | \
        (1u << PIO_SM0_PINCTRL_SIDESET_BASE_LSB))
    static_assert( 0 == (0xff000000u & (PINHI_ALL_PINCTRL_LSBS * 0x1f)), "");
    // note we put the jmp_ctrl pin starting at bit 24
    #define PINHI_ALL_PIN_LSBS ( \
        (1u << 24) | \
        (1u << PIO_SM0_PINCTRL_IN_BASE_LSB) | \
        (1u << PIO_SM0_PINCTRL_OUT_BASE_LSB) | \
        (1u << PIO_SM0_PINCTRL_SET_BASE_LSB) | \
...
static inline void sm_config_set_jmp_pin(pio_sm_config *c, uint pin) {
    ...
    c->execctrl =
        (c->execctrl & ~PIO_SM0_EXECCTRL_JMP_PIN_BITS) |
...
static inline void sm_config_set_fifo_join(pio_sm_config *c, enum pio_fifo_join join) {
    valid_params_if(HARDWARE_PIO,
        join == PIO_FIFO_JOIN_NONE ||
        join == PIO_FIFO_JOIN_TX ||
        join == PIO_FIFO_JOIN_RX
#if PICO_PIO_VERSION > 0
     || join == PIO_FIFO_JOIN_TXPUT ||
        join == PIO_FIFO_JOIN_TXGET ||
        join == PIO_FIFO_JOIN_PUTGET
#endif
    );
#if PICO_PIO_VERSION == 0
    c->shiftctrl =
        (c->shiftctrl & (uint)~(PIO_SM0_SHIFTCTRL_FJOIN_TX_BITS | PIO_SM0_SHIFTCTRL_FJOIN_RX_BITS)) |
        (((uint)join) << PIO_SM0_SHIFTCTRL_FJOIN_TX_LSB);
#else
    c->shiftctrl =
        (c->shiftctrl & 
          (uint)~(PIO_SM0_SHIFTCTRL_FJOIN_TX_BITS | PIO_SM0_SHIFTCTRL_FJOIN_RX_BITS |
...
static inline void sm_config_set_mov_status(pio_sm_config *c, enum pio_mov_status_type status_sel, uint status_n) {
    ...
    c->execctrl =
        (c->execctrl & ~(PIO_SM0_EXECCTRL_STATUS_SEL_BITS | PIO_SM0_EXECCTRL_STATUS_N_BITS)) |
...
static inline void pio_load_program(PIO pio, const uint16_t instructions[], uint from, uint to) {
...
int pio_sm_init(pio_sm_hw_t& sm, uint initial_pc, const pio_sm_config *config);
 */
...
static inline void pio_sm_set_wrap(pio_sm_hw_t& sm, uint wrap_target, uint wrap) {
    ...
    valid_params_if(HARDWARE_PIO, wrap_target < PIO_INSTRUCTION_COUNT);
    sm.execctrl =
            (sm.execctrl & ~(PIO_SM0_EXECCTRL_WRAP_TOP_BITS | PIO_SM0_EXECCTRL_WRAP_BOTTOM_BITS)) |
            (wrap_target << PIO_SM0_EXECCTRL_WRAP_BOTTOM_LSB) |
            (wrap << PIO_SM0_EXECCTRL_WRAP_TOP_LSB);
}
 */
...
#if PICO_PIO_USE_GPIO_BASE
    out_base -= pio_get_gpio_base(sm.pio);
#endif
    valid_params_if(HARDWARE_PIO, out_base < 32);
    valid_params_if(HARDWARE_PIO, out_count <= 32);
    sm.pinctrl =
        (sm.pinctrl & ~(PIO_SM0_PINCTRL_OUT_BASE_BITS | PIO_SM0_PINCTRL_OUT_COUNT_BITS)) |
...
#if PICO_PIO_USE_GPIO_BASE
    set_base -= pio_get_gpio_base(sm.pio);
#endif
    valid_params_if(HARDWARE_PIO, set_base < 32);
    valid_params_if(HARDWARE_PIO, set_count <= 5);
    sm.pinctrl =
        (sm.pinctrl & ~(PIO_SM0_PINCTRL_SET_BASE_BITS | PIO_SM0_PINCTRL_SET_COUNT_BITS)) |
...
#if PICO_PIO_USE_GPIO_BASE
    in_base -= pio_get_gpio_base(sm.pio);
#endif
    valid_params_if(HARDWARE_PIO, in_base < 32);
    sm.pinctrl =
        (sm.pinctrl & ~PIO_SM0_PINCTRL_IN_BASE_BITS) |
 ...
 * \param pindirs True if the side set affects pin directions rather than values
 */
static inline void pio_sm_set_sideset(pio_sm_hw_t& sm, uint bit_count, bool optional, bool pindirs)
{
...
static inline void pio_sm_set_sideset_pins(pio_sm_hw_t& sm, uint sideset_base) {
    ...
    sm.pinctrl =
        (sm.pinctrl & ~PIO_SM0_PINCTRL_SIDESET_BASE_BITS) |
 ...
 * \param pin The raw GPIO pin number to use as the source for a `jmp pin` instruction
 */
static inline void pio_sm_set_jmp_pin(pio_sm_hw_t& sm, uint pin) {
#if PICO_PIO_USE_GPIO_BASE
    pin -= pio_get_gpio_base(sm.pio);
#endif
    valid_params_if(HARDWARE_PIO, pin < 32);
    sm.execctrl =
        (sm.execctrl & ~PIO_SM0_EXECCTRL_JMP_PIN_BITS) |
 ...
 * \param sm pio_sm_hw_t
 */
static inline void pio_sm_clear_fifos(pio_sm_hw_t& sm) {
 ...
 */
static inline void pio_sm_set_consecutive_pindirs(pio_sm_hw_t& sm, uint pin, uint count, bool is_out) {
...
static inline void pio_sm_set_pindir(pio_sm_hw_t& sm, uint pin, bool is_out) {
    uint32_t pinctrl_saved = sm.pinctrl;
    uint pindir_val = is_out ? 0x1f : 0;
