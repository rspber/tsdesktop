...
void TSD_SCREEN::drawChar(cursor_t* cursor, font_t* font, const char c, const rgb_t color)
{
  clip_t clip{0, 0, _width, _height};
  drawChar(&clip, cursor, font, c, color);
}

void TSD_SCREEN::drawTextLine(cursor_t* cursor, font_t* font, const char* text, const rgb_t color, const int8_t spacing)
{
  clip_t clip{0, 0, _width, _height};
  drawTextLine(&clip, cursor, font, text, color, spacing);
}

void TSD_SCREEN::drawText(const int16_t x, const int16_t y, const char* text, const int8_t fontSize, const rgb_t textColor, const int8_t spacing)
{
  cursor_t cursor{x, y};
  font_t font{NULL, fontSize, fontSize};
  drawTextLine(&cursor, &font, text, textColor, spacing);
}
