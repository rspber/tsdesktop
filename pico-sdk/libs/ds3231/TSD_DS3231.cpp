...
void TSD_DS3231::getDateTime(dttm_t* dttm)
{
  uint8_t b[7];
  write8read(0, b, 7);
  dttm->sec = bcdToDec(b[0]);
  dttm->min = bcdToDec(b[1]);
  dttm->hour = bcdToDec(b[2] & 0b111111); //24 hour time
  dttm->dow = bcdToDec(b[3]); //1-7 -> Sunday - Saturday
  dttm->day = bcdToDec(b[4]);
  dttm->month = bcdToDec(b[5] & 0x7f);  // cut century
  dttm->year = bcdToDec(b[6]);
}
